        ...
        <div className={`w-full grid gap-4 p-4 py-6 ${gridClasses}`}>
            {data.map((item, index) => {
                const card = (
                    <Card
                        key={index}
                        className={`rounded-md border shadow-none ${withDescription ? "bg-white" : "bg-transparent border-none"
                            }`}
                    >
                        <CardContent className={`p-0 ${cardContentClass}`}>
                            <div
                                className={`${withDescription ? "h-64 rounded-t-md bg-cover" : "h-24 bg-contain"
                        ...
                        {withDescription && (
                            <CardFooter className="p-2 flex flex-col items-start space-y-2">
                                <p className="text-md quicksand-semibold text-lg">{item.name}</p>
                                <p className="text-md quicksand-regular text-sm">{item.description}</p>
                            </CardFooter>
                        )}
                    </Card>
                );
